...
function imageCandidates(project) {
  ...
  // Самые важные поля первыми: именно они должны быть главным фото объекта.
  ...
  // Все numbered image-поля: image1, image2, image_1 и т.д.
  Object.keys(project)
  ...
  // project.image ставим почти в конец, потому что у тебя там иногда фото категории.
  push(project.image);
...
function getBestImage(project) {
  ...
  return candidates.find((url) => !isBadCategoryImage(url)) || candidates[0] || `${SITE}${BASE_PATH}/images/new-developments-costa-blanca.jpg`;
}
...
<svg width="${WIDTH}" height="${HEIGHT}" viewBox="0 0 ${WIDTH} ${HEIGHT}" xmlns="http://www.w3.org/2000/svg">
  ...
  <rect width="1200" height="630" fill="#183746"/>
  <rect width="1200" height="630" fill="url(#shade)"/>

  <rect x="58" y="396" width="260" height="46" rx="23" fill="#ffffff" fill-opacity="0.18" stroke="#ffffff" stroke-opacity="0.26"/>
  ...
  <text x="60" y="510" font-family="Arial, Helvetica, sans-serif" font-size="62" font-weight="900" fill="#ffffff" filter="url(#shadow)">${safeTitle}</text>
  <text x="60" y="572" font-family="Arial, Helvetica, sans-serif" font-size="38" font-weight="900" fill="url(#gold)">${safePrice}</text>
  <text x="60" y="612" font-family="Arial, Helvetica, sans-serif" font-size="24" font-weight="700" fill="#ffffff" fill-opacity="0.88">${safeMeta}</text>
</svg>`);
}
...
async function createOgImage(project) {
  ...
  try {
    ...
    await sharp(imageBuffer)
      ...
      .modulate({ brightness: 0.88, saturation: 0.98 })
      .composite([{ input: makeOgOverlay({ title, price, meta }), top: 0, left: 0 }])
      .jpeg({ quality: 92, progressive: true })
      .toFile(outputPath);
